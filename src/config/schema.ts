...
export const ExecutorBackendConfigSchema = z.object({
  ...
  protocol: z.enum(['cli-stdout', 'cli-json', 'acp', 'mcp']).default('cli-json'),
...
export const ContextCappingStrategySchema = z.object({
  ...
  relevanceRanking: z.array(z.string()).default([
    'design.md',
    'tech-spec.md',
    'task.md',
    'ARCHITECTURE.md',
...
export type ContextCappingStrategy = z.infer<typeof ContextCappingStrategySchema>;
...
export const ReviewCouncilConfigSchema = z.object({
  ...
  perspectives: z.array(z.enum(['security', 'performance', 'maintainability'])).default([
    'security',
    'performance',
    'maintainability'
  ])
...
export const KASOConfigSchema = z.object({
  ...
  backendSelectionStrategy: z.enum(['default', 'context-aware']).default('default'),
  
  // Concurrency
  maxConcurrentAgents: z.union([z.literal('auto'), z.number().int().positive()]).default('auto'),
