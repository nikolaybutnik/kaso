  ...
  for (const secret of sortedSecrets) {
    if (secret && secret.length > 0) {
      ...
      try {
        ...
        console.warn(`Failed to create regex for secret: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
 ...
 */
export function redactMultiple(sources: string[], secrets: Set<string>): string[] {
 ...
 */
export function redactObject(obj: unknown, secrets: Set<string>): string | null | undefined {
 ...
 */
export function redactError(error: Error, secrets: Set<string>): {
  message: string;
  stack?: string;
} {
  const redacted = {
