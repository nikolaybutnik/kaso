...
import { redactSecrets } from '../../src/infrastructure/log-redactor.js';
...
describe('Property 36: Secret redaction in logs', () => {
  ...
  ])('should redact all secrets from any log text', async (secretsArray, baseText) => {
    // Skip if any secret is just whitespace or has special regex chars
    const hasValidSecrets = secretsArray.every(s => s.trim().length > 0 && !/[*+?^$()|[\]{},%]/.test(s));
    if (!hasValidSecrets) {
  ...
  test.prop([
    fc.array(fc.string({ minLength: 10, maxLength: 30 }), { minLength: 1, maxLength: 3 })
...
describe('Property 37: Credentials loaded only from secure sources', () => {
  test.prop([
    ...
    fc.array(fc.string({ minLength: 5, maxLength: 30 }), { minLength: 1, maxLength: 5 })
  ])('should only load from environment or keychain, never files', async (envKeys, keychainKeys) => {
    // Skip if any key is just whitespace or has special regex chars
    const allKeyValues = [...envKeys, ...keychainKeys];
    const hasValidKeys = allKeyValues.every(s => s.trim().length > 0 && !/[*+?^$()|[\]{},%]/.test(s));
    if (!hasValidKeys) {
      return; // Skip this test case
    }
    const credentialManager = new CredentialManager();
  ...
  ])('should prefer environment variables over keychain', async (secretValue) => {
    const credentialManager = new CredentialManager();
  ...
  test('should throw error for missing keys without attempting file access', async () => {
    ...
    await expect(credentialManager.getApiKey(keyName))
      .rejects
      .toThrow(/not found/);
    
    // Should not contain file access attempts
    expect(credentialManager.getApiKey.toString()).not.toMatch(/readFile|readFileSync|require\(['"]\.\/|require\(['"]\.\.\//);
  });
  ...
  test.prop([
    ...
    // Load once
    await Promise.all(secrets.map((_, i) => credentialManager.getApiKey(`CACHE_KEY_${i}`)));
    
    // Clear spy to see if functions are called again
    getPasswordSpy.mockClear();
    
    // Load again
    const secondLoad = await Promise.all(secrets.map((_, i) => credentialManager.getApiKey(`CACHE_KEY_${i}`)));
