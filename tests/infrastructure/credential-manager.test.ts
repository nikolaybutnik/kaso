...
describe('CredentialManager', () => {
  ...
  describe('getApiKey', () => {
    ...
    it('should throw descriptive error if key not found', async () => {
      ...
      await expect(credentialManager.getApiKey('TEST_API_KEY'))
        .rejects
        .toThrow('API key "TEST_API_KEY" not found');
    });
    ...
    it('should throw error if keychain access fails', async () => {
      ...
      await expect(credentialManager.getApiKey('TEST_API_KEY'))
        .rejects
        .toThrow('Failed to access OS keychain');
    });
  });
  ...
  describe('listRequiredKeys', () => {
    ...
    it('should return configured required keys', () => {
      ...
      expect(credentialManager.listRequiredKeys()).toEqual(['KEY1', 'KEY2', 'KEY3']);
    });
  });
  ...
  describe('validateAllPresent', () => {
    ...
    it('should throw error if any required key is missing', async () => {
      ...
      await expect(credentialManager.validateAllPresent())
        .rejects
        .toThrow('Missing required API keys: KEY2');
    });
    ...
    it('should list all missing keys in error', async () => {
      ...
      await expect(credentialManager.validateAllPresent())
        .rejects
        .toThrow('Missing required API keys: KEY1, KEY2');
    });
  });
  ...
  describe('redact', () => {
    ...
    it('should redact secrets from text', async () => {
      ...
      const text = 'Error: failed to authenticate with secret-key-abc-123 and another-secret-key-xyz-456';
      const redacted = credentialManager.redact(text);
      
      expect(redacted).toBe('Error: failed to authenticate with [REDACTED] and [REDACTED]');
