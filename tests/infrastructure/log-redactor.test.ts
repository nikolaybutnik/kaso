...
import { redactSecrets, redactMultiple, redactObject, redactError } from '../../src/infrastructure/log-redactor.js';

describe('redactSecrets', () => {
  it('should redact secrets from text', () => {
    const secrets = new Set(['secret-key-123', 'password456']);
    const text = 'Error: failed to authenticate with secret-key-123 and password456';
    
    const result = redactSecrets(text, secrets);
    
    expect(result).toBe('Error: failed to authenticate with [REDACTED] and [REDACTED]');
  ...
  it('should redact all occurrences of each secret', () => {
    ...
    expect(result).toBe('Key 1: [REDACTED], Key 2: [REDACTED], Key 3: [REDACTED]');
  ...
  it('should redact API keys that look like real keys', () => {
    ...
    const text = 'Authenticating with sk-ant-api03-very-long-key-with-many-characters-123456789 and xai-very-long-key-with-dashes-and-numbers-0987654321';
...
describe('redactError', () => {
  it('should redact secrets from error message and stack', () => {
    ...
    const error = new Error('Authentication failed with secret-key and password');
    
    const result = redactError(error, secrets);
    
    expect(result.message).toBe('Authentication failed with [REDACTED] and [REDACTED]');
...
describe('Property 36: Secret redaction in logs', () => {
  it('should redact all secrets without exception', () => {
    ...
    const logMessage = 'Processing with secret1, secret2, and secret3 for operation';
